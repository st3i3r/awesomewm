...
volumebar_widget:connect_signal("button::press", function(_,_,_,button)
    if (button == 4)     then awful.spawn("amixer -D default sset Master 5%+", false)
    elseif (button == 5) then awful.spawn("amixer -D default sset Master 5%-", false)
    elseif (button == 1) then awful.spawn("amixer -D default sset Master toggle", false)
    end
